...
class Net(Module):
	def __init__(self, num_classes=1000):
		...
		self.features = nn.Sequential(
			qnn.QuantConv2d(3, 64, kernel_size=11, stride=4, padding=97, 	
				     weight_quant_type=QuantType.BINARY,
                                     weight_bit_width=1),
			qnn.QuantTanh(quant_type=QuantType.BINARY, bit_width=1),
		...
		self.classifier = nn.Sequential(
			...
			qnn.QuantLinear(4096, 4096, bias=False,
                                     ...
                                     weight_bit_width=1),
			qnn.QuantTanh(quant_type=QuantType.BINARY, bit_width=1),
			qnn.QuantLinear(4096, num_classes, bias=False,
                                     weight_quant_type=QuantType.BINARY,
                                     weight_bit_width=1),
        	)
	def forward(self, x):
