...
for x in choices:
    ...
    for x in range(2,9):
        ...
        weightQuantType = QuantType.FP
        weightBitWidth = x
        activationQuantType = QuantType.FP
        activationBitWidth = x
        ...
        class Net(nn.Module):
            def __init__(self, num_classes = num_classes, weight_quant_type=weightQuantType, weight_bit_width=weightBitWidth, 
                ...
                self.features = nn.Sequential(
                    qnn.QuantConv2d(3, 64, kernel_size=11, stride=4, padding=0,
                        weight_quant_type=weightQuantType, weight_bit_width=weightBitWidth),
                    qnn.QuantTanh(quant_type=activationQuantType, bit_width=activationBitWidth),
                    nn.MaxPool2d(kernel_size=3, stride=2),
                    qnn.QuantConv2d(64, 192, kernel_size=5, padding=0,
                        weight_quant_type=weightQuantType, weight_bit_width=weightBitWidth),
                    qnn.QuantTanh(quant_type=activationQuantType, bit_width=activationBitWidth),
                    nn.MaxPool2d(kernel_size=3, stride=2),
                    qnn.QuantConv2d(192, 384, kernel_size=3, padding=0,
                        weight_quant_type=weightQuantType, weight_bit_width=weightBitWidth),
                    qnn.QuantTanh(quant_type=activationQuantType, bit_width=activationBitWidth),
                    qnn.QuantConv2d(384, 256, kernel_size=3, padding=0,
                        weight_quant_type=weightQuantType, weight_bit_width=weightBitWidth),
                    qnn.QuantTanh(quant_type=activationQuantType, bit_width=activationBitWidth),
